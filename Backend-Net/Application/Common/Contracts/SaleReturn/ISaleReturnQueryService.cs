namespace Application.Common.Contracts.SaleReturn
{
    ...
    /// and the SaleReturn/SaleReturnClaim/SaleReturnItem roll-up properties all sum over the loaded
    /// graph, so a handler that forgets a ThenInclude does not fail - it silently computes a status
    /// from empty collections and persists it. Any handler that recomputes a status must load
    ...
    public interface ISaleReturnQueryService
    {
        ...
        /// <summary>
        /// Claims -> (Product, InspectionItems -> Decisions): everything the return's own math reads.
        /// <paramref name="includeSaleItems"/> adds Sale -> Items, which only handlers that settle
        /// quantity against the sale need (AddSaleReturnDecision) - see the note on
        /// <see cref="ISaleReturnCalculationService.RecomputeSaleStatus"/>.
        /// </summary>
