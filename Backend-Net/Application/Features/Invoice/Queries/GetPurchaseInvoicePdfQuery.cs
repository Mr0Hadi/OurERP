...
namespace Application.Features.Invoice.Queries
{
    ...
    public class GetPurchaseInvoicePdfQueryHandler : IRequestHandler<GetPurchaseInvoicePdfQuery, FileResponseDto>
    {
        ...
        public GetPurchaseInvoicePdfQueryHandler(IWMSDbContext context, IPdfDocumentService pdfDocumentService, IConfiguration configuration)
        {
            _context = context;
        ...
        public async Task<FileResponseDto> Handle(GetPurchaseInvoicePdfQuery request, CancellationToken cancellationToken)
        {
            ...
            var lines = purchase.Items.Select((item, index) => InvoiceLineCalculator.Build(
                index + 1,
                item.Product.Code,
                item.Product.Name,
                item.Quantity,
