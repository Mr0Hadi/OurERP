...
function generateMorePurchases(count = 20) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const purchase = {
      ...
      invoiceNumber: `INV-2026-${String(purchasesMock.length + i + 1).padStart(3, '0')}`,
    ...
    if (paymentType === PAYMENT_TYPES.CHECK) {
      purchase.checkNumber = String(Math.floor(Math.random() * 9000000000) + 1000000000);
    } else if (paymentType === PAYMENT_TYPES.TRANSFER) {
      purchase.transferRef = `TRN-${Math.floor(Math.random() * 90000000) + 10000000}`;
    }
