...
import {
  ...
  ArrowUpDown, ArrowUp, ArrowDown, Truck, Undo2,
} from "lucide-react";
...
import { OUTGOING_TYPES, OUTGOING_TYPE_LABELS } from "../../services/outgoingQueueApi";

const PAGE_SIZE_OPTIONS = [5, 10, 20, 30, 50];

const TYPE_STYLES = {
  [OUTGOING_TYPES.SALE]: "bg-blue-100 text-blue-800 border-blue-300 hover:bg-blue-100",
  [OUTGOING_TYPES.RETURN_REPLACEMENT]: "bg-indigo-100 text-indigo-800 border-indigo-300 hover:bg-indigo-100",
};
const TYPE_ICON = { [OUTGOING_TYPES.SALE]: Truck, [OUTGOING_TYPES.RETURN_REPLACEMENT]: Undo2 };

const TypeBadge = ({ type }) => {
  const Icon = TYPE_ICON[type] ?? Truck;
  return (
    <Badge className={`gap-1 ${TYPE_STYLES[type] ?? "bg-gray-100 text-gray-800"}`}>
...
const ShippingTable = ({
  data, isLoading, totalPages, currentPage, pageSize,
  onPaginationChange, sorting, onSortingChange,
}) => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "تعداد اقلام",
        cell: (info) => <span className="tabular-nums text-sm">{info.getValue().toLocaleString("fa-IR")}</span>,
      },
      {
      ...
      },
      {
        ...
        enableSorting: false,
        cell: ({ row }) => {
          const isReplacement = row.original.type === OUTGOING_TYPES.RETURN_REPLACEMENT;
          const path = isReplacement
            ? ROUTES.WAREHOUSE_SHIPPING_REPLACEMENT_DETAIL.replace(":returnId", row.original.returnId)
            : ROUTES.WAREHOUSE_SHIPPING_DETAIL.replace(":id", row.original.saleId);
          return (
            <Button variant="outline" size="sm" onClick={() => navigate(path)} className="gap-1">
    ...
    <div className="space-y-3">
        ...
        <div className="rounded-md border overflow-x-auto">
          <Table>
                    ...
                    return (
                      <TableHead key={header.id} className="text-center">
                        {header.isPlaceholder ? null : (
                          <div
                            className={isSortable ? "flex items-center justify-center gap-1 cursor-pointer select-none hover:text-foreground transition-colors" : "flex items-center justify-center gap-1"}
            ...
            <TableBody>
              {rows.length ? (
                rows.map((row) => (
                  <TableRow key={row.original.id} className="hover:bg-muted/50 transition-colors">
      ...
      <div className="flex flex-col gap-2 sm:flex-row items-center justify-between px-2">
        <div className="flex items-center gap-2">
          ...
          <Select value={pageSize.toString()} onValueChange={(value) => onPaginationChange({ pageIndex: 0, pageSize: Number(value) })}>
            <SelectTrigger className="h-8 w-[70px]"><SelectValue /></SelectTrigger>
            <SelectContent side="top">
              {PAGE_SIZE_OPTIONS.map((size) => (<SelectItem key={size} value={size.toString()}>{size.toLocaleString("fa-IR")}</SelectItem>))}
            </SelectContent>
          </Select>
        </div>
