// src/features/sales/components/forms/SalesReturnItemsSection.jsx
  ...
  return (
    <div className={`border border-border rounded-lg p-3 space-y-2.5 ${allocated > 0 ? "bg-primary/[0.03]" : ""}`}>
      ...
      >
        ...
        افزودن دلیل برای بخشی از این کالا
      </Button>

      {claims.length === 0 && (
        <p className="text-xs text-muted-foreground">
          اگر مشتری چیزی از این کالا را برنمی‌گرداند، نیازی به کاری نیست.
        </p>
      )}
    </div>
  );
...
export default function SalesReturnItemsSection({ items, onAddClaim, onUpdateClaim, onRemoveClaim }) {
    ...
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-semibold text-card-foreground">اقلام قابل مرجوع‌کردن</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-center text-sm text-muted-foreground py-6 border border-dashed border-border rounded-lg">
            برای این فاکتور، هیچ کالای قابل مرجوع‌کردنی باقی نمانده است
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-card-foreground">اقلام قابل مرجوع‌کردن</CardTitle>
        <p className="text-xs text-muted-foreground">
          برای هر کالا می‌توانید تعداد را بین چند دلیل مختلف تقسیم کنید — مثلاً بخشی آسیب‌دیده و بخشی
          دیگر تاریخ‌گذشته باشد. سقف هر کالا بر اساس مقداری است که واقعاً تحویل مشتری شده و هنوز مرجوعی
          فعالی برایش ثبت نشده.
        </p>
      </CardHeader>
