...
function PurchaseReturnDetailForm({ purchaseReturn }) {
  ...
  const addResolutionMutation = useAddReturnItemResolutionMutation(purchaseReturn.id);
  const removeResolutionMutation = useRemoveReturnItemResolutionMutation(purchaseReturn.id);
  ...
  const statusConfig =
    STATUS_CONFIG[purchaseReturn.status] ?? STATUS_CONFIG[PURCHASE_RETURN_STATUSES.PENDING];
  const StatusIcon = statusConfig.icon;

  // ─── خلاصه مالی ────────────────────────────────────────────────────────
  const allLines = purchaseReturn.items.flatMap((i) => i.resolutions || []);
  const resolvedLines = allLines.filter((l) => l.status === RESOLUTION_LINE_STATUSES.RESOLVED);
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <p className="text-xs text-muted-foreground">
                این اطلاعات از گزارش انبار در لحظه‌ی ثبت این مرجوعی گرفته شده. تصمیم‌گیری و تسویه‌ی
                هر قلم در بخش «پیگیری و هماهنگی» انجام می‌شود.
              </p>
            </CardHeader>
...
<div className="hidden md:block border border-border rounded-lg overflow-hidden">
  <table className="w-full table-fixed text-sm">
    <thead className="bg-muted text-muted-foreground text-xs">
      <tr>
        <th className="w-[34%] text-right px-3 py-2.5 font-medium">کالا</th>
        <th className="w-[12%] text-center px-2 py-2.5 font-medium">تعداد</th>
        <th className="w-[16%] text-center px-2 py-2.5 font-medium">قیمت واحد</th>
        <th className="w-[22%] text-center px-2 py-2.5 font-medium">نوع مشکل گزارش‌شده</th>
        <th className="w-[16%] text-center px-2 py-2.5 font-medium">جمع</th>
      </tr>
    </thead>
    <tbody className="divide-y divide-border">
      {purchaseReturn.items.map((item) => (
        <tr key={item.issueId} className="hover:bg-accent/30 transition-colors">
          <td className="px-3 py-2 truncate">
            <p className="font-medium text-card-foreground text-sm truncate">
              {item.productName}
            </p>
            <p className="text-xs text-muted-foreground truncate">{item.productCode}</p>
            {item.note && (
          ...
          <td className="px-2 py-2 text-center">
            <Badge variant="outline" className="text-[11px] whitespace-normal">
          ...
          {purchaseReturn.totalAmount.toLocaleString("fa-IR")}
              ...
              <div className="md:hidden space-y-2">
                {purchaseReturn.items.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground border-t border-border/60 pt-2">
                      <span>
                        تعداد: {item.qty.toLocaleString("fa-IR")} × {item.unitPrice.toLocaleString("fa-IR")}
                      </span>
                ...
                <div className="flex items-center justify-between rounded-lg bg-muted px-3 py-2.5 border border-border">
                  <span className="text-sm font-medium text-muted-foreground">جمع کل:</span>
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className={`gap-1.5 ${statusConfig.className}`}>
                  ...
                  {PURCHASE_RETURN_STATUS_LABELS[purchaseReturn.status] ?? purchaseReturn.status}
                </Badge>
          ...
          <Card>
            ...
            <CardContent>
              <div className="rounded-lg bg-muted/50 border border-border p-3 space-y-2 text-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">بازگشت وجه نهایی‌شده</span>
                ...
                {writeOffAmount > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">زیان پذیرفته‌شده</span>
                ...
                <div className="flex justify-between items-center border-t border-border pt-2">
                  <span className="text-muted-foreground">مانده در انتظار تصمیم</span>
                  <span
                    className={`font-semibold ${
                      pendingQty > 0 ? "text-amber-600 dark:text-amber-400" : "text-[oklch(0.50_0.16_152)]"
                    }`}
                  >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              پس از حذف، این کسری دوباره با وضعیت «قابل پیگیری» در لیست ظاهر می‌شود.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
