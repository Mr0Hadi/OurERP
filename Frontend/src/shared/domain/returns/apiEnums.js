 ...
 *
 * ۳. اگر روزی تصمیم گرفته شود enumها روی سیم *عددی* منتقل شوند، جای
 *    جدولِ تبدیل دقیقاً همین‌جاست و هیچ فایل دیگری دست نمی‌خورد.
 *    (پیشنهاد فعلی رشته است — دلایلش در
 *    `docs/returns/phase3-backend-spec.fa.md` بند ۳.۳.)
 *
...
function verify(path, value, allowed) {
  ...
  const hint =
    typeof value === "number"
      ? "سرور enum را عددی فرستاده؛ قرارداد رشته‌ای است (JsonStringEnumConverter)."
