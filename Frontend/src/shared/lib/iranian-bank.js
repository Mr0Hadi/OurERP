...
export function normalizeShaba(value) {
...
export function formatShaba(value, separator = " ") {
  return (normalizeShaba(value)
    .match(/.{1,4}/g)
    ?.join(separator) ?? "");
}
...
export function getIranianBankByShaba(value) {
  const digits = normalizeShaba(value)
  return iranianBanks.find((bank) =>
    bank.ibanCodes.includes(digits.slice(2, 5)));
}

export function validateIranianCard(value) {
  const digits = normalizeCardNumber(value)
...
export function validateIranianShaba(value) {
