 ...
 * یک قلمِ لیست.
 *
 * چرا کامپوننتِ جداست و نه فقط یک `<li>` داخلِ `map`: قلمی که از سرور
 * فقط `objectKey` گرفته باشد باید آدرسش را خودش بگیرد، و این با هوک
 * انجام می‌شود.
 *
 * قبلاً این‌جا فقط `item.url` مصرف می‌شد و چون `openable` به وجودِ
 * `src` گره خورده، هر قلمِ بی‌آدرس نه بندانگشتی داشت و نه حتی دکمه‌ی
 * بازکردن — فایل ثبت شده بود ولی راهی برای دیدنش نبود.
 *
 * رفتار عمداً همان `RemoteImage` است: وقتی پاسخ خودش آدرس داده هیچ
 * درخواستِ اضافه‌ای زده نمی‌شود، و آدرسِ گرفته‌شده در کشِ مشترک می‌نشیند
 * تا هرجای دیگری که همان کلید را نشان می‌دهد هم از آن استفاده کند.
 */
function FileUploadListItem({
  ...
  const { data: fetchedUrl } = useImageUrlQuery(item.objectKey, {
    enabled: Boolean(item.objectKey) && !item.localPreview && !item.url,
  });

  // پیش‌نمایشِ محلی مقدم است: تا وقتی آپلود تمام نشده چیزی روی سرور
  // نیست که آدرسی داشته باشد.
  const src = item.localPreview || item.url || fetchedUrl || null;
