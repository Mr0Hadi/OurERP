 ...
 * یک قلمِ لیست.
 *
 * چرا کامپوننتِ جداست و نه یک `<li>` داخلِ `map`: قلمی که از سرور فقط
 * `objectKey` گرفته باشد باید آدرسش را خودش بگیرد، و گرفتن یعنی هوک.
 * بدون این، قلمِ بی‌آدرس نه بندانگشتی دارد و نه دکمه‌ی بازکردن (چون
 * `openable` به وجودِ `src` گره خورده) — فایل هست ولی دیده نمی‌شود.
 *
 * رفتار عمداً همان `RemoteImage` است: اگر پاسخ خودش آدرس داده باشد
 * درخواستِ اضافه‌ای نمی‌رود، و آدرسِ گرفته‌شده در کشِ مشترک می‌نشیند تا
 * هرجای دیگری که همان کلید را نشان می‌دهد از آن استفاده کند.
 */
