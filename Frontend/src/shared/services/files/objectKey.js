 ...
 * کامپوننت‌های نمایشی‌اند (`RemoteImage`). وقتی داخل فایلِ axios بودند،
 ...
 * شده بود.
 *
 * ⚠️ قراردادِ آدرس عوض شده. تصاویر دیگر با یک URLِ امضاشده‌ی موقتِ باکت
 * نمایش داده نمی‌شوند: باکتِ Liara به هر درخواستی که User-Agentش شبیه
 * مرورگر باشد `404 page not found` می‌دهد، پس هیچ آدرسِ باکتی در
 * `<img src>` کار نمی‌کند. حالا خودِ API بایت‌ها را سرو می‌کند و آدرسی
 * که می‌دهد ثابت است و منقضی نمی‌شود:
 *
 *   {PublicBaseUrl}/api/File/GetImage?objectKey={key}
 *
 * دو نتیجه‌ی مستقیم برای این فایل:
 *  ۱. کلید در *کوئری‌استرینگ* است نه در مسیر — خواندنِ `pathname` مثل
 *     قبل، `api/File/GetImage` را به‌عنوان کلید برمی‌گرداند.
 *  ۲. اگر `PublicBaseUrl` در سرور خالی باشد، آدرس *نسبی* می‌آید
 *     (`/api/File/GetImage?...`) که برای فرانتِ روی دامنه‌ی دیگر بی‌معنی
 *     است؛ پس نسبت به هاستِ API حل می‌شود.
 */
